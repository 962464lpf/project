      ...
      <div class="main-center">
        <el-table
          ...
          <el-table-column
            ...
            prop="requireFile">
            <template slot-scope="scope">
              <upload-btns :edit="scope.row.finish === 0" :filePaths="scope.row.requireFile" @click="uploadBtnsClick(21, scope.row, $event, '运营阶段的绿色要点及要求')" />
            </template>
          </el-table-column>
        </el-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    uploadBtnsClick(fileType, row, event, title) {
      if (event === "view") {
        const id = row.id
        const path = '/buildEnvironment/build/trackManagement/fileDetail'
        this.$router.push({
          'path': path,
          'query': {
            id,
