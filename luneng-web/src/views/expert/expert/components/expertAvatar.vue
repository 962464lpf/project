<template>
  <div>
    <div class="imgBox" @click="editCropper()">
      <img v-if="options.img"
        :src="options.img"
        class="img-circle img-lg "/>
      <div v-else>
        <el-button type="text">点击上传照片</el-button>
      </div>
    </div>
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body :close-on-click-modal="false">
      <el-row>
        <el-col :xs="24" :md="12" :style="{height: '350px'}">
          <vue-cropper
            ref="cropper"
            :img="options.img"
            :info="true"
            :autoCrop="options.autoCrop"
            :autoCropWidth="options.autoCropWidth"
            :autoCropHeight="options.autoCropHeight"
            :fixedBox="options.fixedBox"
            @realTime="realTime"
          />
        </el-col>
        <el-col :xs="24" :md="12" :style="{height: '350px'}">
          <div class="avatar-upload-preview">
            <img :src="previews.url" :style="previews.img" />
          </div>
        </el-col>
      </el-row>
      <br />
      <el-row>
        <el-col :lg="2" :md="2">
          <el-upload
            ref="elUploadFile"
            action="#"
            :accept="accept"
            :limit="limit"
            :show-file-list="false"
            :before-upload="beforeUpload">
            <el-button >
              上传
              <i class="el-icon-upload el-icon--right"></i>
            </el-button>
          </el-upload>
          <div class="el-upload__tip" style="color: red; width: 80px; font-size: 12px">
            仅能上传一张
          </div>
        </el-col>
        <el-col :lg="{span: 1, offset: 2}" :md="2">
          <el-button icon="el-icon-plus"  @click="changeScale(1)"></el-button>
        </el-col>
        <el-col :lg="{span: 1, offset: 1}" :md="2">
          <el-button icon="el-icon-minus"  @click="changeScale(-1)"></el-button>
        </el-col>
        <el-col :lg="{span: 1, offset: 1}" :md="2">
          <el-button icon="el-icon-refresh-left"  @click="rotateLeft()"></el-button>
        </el-col>
        <el-col :lg="{span: 1, offset: 1}" :md="2">
          <el-button icon="el-icon-refresh-right"  @click="rotateRight()"></el-button>
        </el-col>
        <el-col :lg="{span: 2, offset: 6}" :md="2">
          <el-button type="primary"  @click="uploadImg()">提 交</el-button>
        </el-col>
      </el-row>
    </el-dialog>
  </div>
</template>

<script>
import { VueCropper } from "vue-cropper";
import { uploadFile } from "@/api/common/utils";

export default {
  components: { VueCropper },
  props: {
    imgFileUrl: {
      type: String,
      default: undefined
    },
    title: {
      type: String,
      default: '上传照片'
    }
  },
  data() {
    return {
      // 是否显示弹出层
      open: false,
      // 弹出层标题
      options: {
        img: undefined, //裁剪图片的地址
        autoCrop: true, // 是否默认生成截图框
        autoCropWidth: 200, // 默认生成截图框宽度
        autoCropHeight: 200, // 默认生成截图框高度
        fixedBox: true // 固定截图框大小 不允许改变
      },
      previews: {},
      limit: 1,
      accept: '.jpg, .jpeg, .png'
    }
  },
  created() {
    this.options.img = this.imgFileUrl
  },
  watch: {
    imgFileUrl(newVal) {
      if(newVal) {
        this.options.img = newVal
      }
    }
  },
  methods: {
    // 编辑头像
    editCropper() {
      this.open = true
    },
    // 向左旋转
    rotateLeft() {
      this.$refs.cropper.rotateLeft();
    },
    // 向右旋转
    rotateRight() {
      this.$refs.cropper.rotateRight();
    },
    // 图片缩放
    changeScale(num) {
      num = num || 1;
      this.$refs.cropper.changeScale(num);
    },

    // 上传预处理
    beforeUpload(file) {
      if (file.type.indexOf("image/") === -1) {
        this.msgError("文件格式错误，请上传图片类型,如：JPG，PNG后缀的文件。");
      } else {
        const reader = new FileReader();
        reader.readAsDataURL(file);
        reader.onload = () => {
          this.options.img = reader.result;
        };
      }
    },

    // 上传图片
    uploadImg() {
      this.$refs.cropper.getCropBlob(data => {
        let formData = new FormData();
        formData.append("file", data);
        uploadFile(formData).then(response => {
          if (response.code === 200) {
            this.open = false;
            this.options.img = response.url
            this.msgSuccess("照片上传成功");
            this.$emit("uploadImgSucc", response.url)
          }
          this.$refs.cropper.clearCrop();
          this.$refs.elUploadFile.clearFiles();
        })
      })
    },
    // 实时预览
    realTime(data) {
      this.previews = data;
    }
  }
}
</script>
<style lang="scss" scoped>
  .imgBox {
    width: 125px;
    display: flex;
    flex-direction: column;
    align-items: center;
    cursor: pointer;
    img {
      border: 1px solid rgba(189,194,201, .3);
    }
  }
</style>
