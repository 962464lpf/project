              ...
              <table class="history-table">
                <tr v-for="item in history">
                  ...
                  <td style="width: 100px; text-align: center">
                    <el-button type="text">
                      <router-link
                        :to="{ path: '/msg/selNotice/detailProHis',query:{id:item.id,hidden: true,from:'his',toId:id} }">
                        查看
                      </router-link>
                    </el-button>
                  </td>
...
import {searchDeclarable, updateSelDeclareInfo} from "@/api/sel/selDeclareInfo";
...
import {addProjectStorageInfo, updateProjectStorageInfo} from "@/api/storage/projectStorageInfo";
import {getCompanyName} from "@/api/system/dept";

export default {
  name: "msg-proReserve-detail",
  data() {
    return {
      tabIndex: '1',
      // 表单参数
  ...
  methods: {
    ...
    /** 修改按钮操作 */
    handleUpdate(row) {
      const path = '/msg/selNotice/editPro'
      const id = row.id
      this.$router.push({
        path,
        query: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除?', "系统提示", {
        ...
        this.closeView('/msg/selNotice/detailPro', '/msgSelNotice', 0)
      }).catch(() => {
      });
    },
  ...
  watch: {
    '$route': function (to, from) {
      if (to.name === 'msg-proReserve-detail') {
