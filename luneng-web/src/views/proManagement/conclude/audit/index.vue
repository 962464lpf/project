<template>
  <div class="app-container">
    ...
    <el-table ref="table" v-loading="loading" :data="resultList">
      ...
      <el-table-column label="结题审核" align="center" class-name="small-padding fixed-width" fixed="right" width="120">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="scope.row.concludeStatus==2"
            v-hasPermi="['conclude:researchResult:audit']"
            @click="handleAudit(scope.row)"
          >审核
          </el-button>
          <el-button
            v-if="scope.row.concludeStatus==3"
            type="text"
            v-hasPermi="['conclude:researchResult:auditDetail']"
...
<script>
  import {auditList, operation} from "@/api/conclude/result"
  ...
  export default {
    ...
    methods: {
      ...
      /** 详情按钮操作 */
      handleDetail(row) {
        operation({"infoId": row.id, "operation": "3"}).then(res => {
          if (res.code == 200) {
            this.$router.push({
