    ...
    <el-row :gutter="20">
      ...
      <el-col :span="19" :xs="24">
        ...
        <el-table ref="table" v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column min-width="120" label="登录名" align="center" prop="userName" />
          <el-table-column min-width="120" label="姓名" align="center" prop="nickName" />
          <el-table-column min-width="230" label="单位" align="center" prop="unitName" />
          <el-table-column min-width="160" label="部门" align="center" prop="dept.deptName" />
          ...
          <el-table-column min-width="120" label="邮箱" align="center" prop="email" />
          <el-table-column min-width="125" label="座机" align="center" prop="mobile" />
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
          ...
          <el-col :span="24">
            <el-form-item label="归属部门" prop="deptId">
              <div class="treeselect-container">
                <treeselect v-model="form.deptId" :options="deptOptions" :multiple="false" placeholder="请选择归属部门"/>
              </div>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="座机" prop="mobile">
              ...
              <!--              <a-input v-model="form.mobile" type="landline" placeholder="请输入座机号码" maxlength="50"/>-->
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="12">
           ...
           <el-col :span="12">
             <el-form-item label="有效时间" prop="expiryDate">
               <el-date-picker clearable
                               style="width: 100%"
                               v-model="form.expiryDate"
                               type="date"
                               :picker-options="pickerExpiryDate"
                               placeholder="为空默认为永久有效"
                               value-format="yyyy-MM-dd">
               </el-date-picker>
              </el-form-item>
            </el-col>
...
<script>
  ...
  import {sysUserRules} from "@/utils/el_validate"
  ...
  export default {
    ...
    created() {
      ...
      sysUserRules.mobile.push({
        validator: (rules, value, callback) => {
          let {areaCode, landlineNumber} = this.form;
          let mobile = null;
          mobile = areaCode + '-' + landlineNumber;
          setTimeout(() => {
            if (!this.areaCodeIsFocus && !this.landlineNumberIsFocus) {
              if (!areaCode && !landlineNumber) return callback();
    ...
    methods: {
      ...
      handleAdd() {
        ...
        getUser().then(response => {
          ...
          this.form.postIds = [this.postOptions[0].postId];
          if (this.form.mobile) {
            if (this.form.mobile.toString().indexOf('-') !== -1) {
              let arr = this.form.mobile.split('-');
              if (arr.length > 1) {
                this.form.areaCode = arr[0];
                this.form.landlineNumber = arr[1];
              }
            }
          } else {
            this.form.areaCode = '';
            this.form.landlineNumber = '';
          }
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出所有用户数据项?', "系统提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function () {
